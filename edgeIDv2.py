...
def heuristicMinCut2(g, S, postProcess=False):     # break the ancestral sets where bidir(S) are present.
    comps = nx.connected_components(g.nodeSubgraph(S, directed=False))  # C-components of S
    comps = [c for c in comps]
    dirP = []
    for comp in comps:
...
# returns a set which is sufficient to intervene upon for identification of Q[S], along with its cost.
# followed by an optional heuristic post-process to reduce the cost
def heuristicMinCut1(g, S, postProcess=False):     # break the c-components where pa(S) are present.
    comps = nx.connected_components(g.nodeSubgraph(S, directed=False))  # C-components of S
    comps = [c for c in comps]
    dirP = []
    for comp in comps:
...
def naiveGreedy(g, S, postProcess=False):  # Reduce the sum of the cost of the remaining hedge hull
    comps = nx.connected_components(g.nodeSubgraph(S, directed=False))  # C-components of S
    comps = [c for c in comps]
    dirP = []
    for comp in comps:
...
def edgeIDbrute(g, Y):
    return 'brute force not implemented'
...
if __name__ == '__main__':
    ...
    intervention, cost = naiveGreedy(h, y_hat)            # a naive greedy for min-cost intervention
